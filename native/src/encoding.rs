...
pub fn native_encode_base64(input: Buffer, padded: Option<bool>, url_safe: Option<bool>) -> String {
	...
	let dict = if url_safe { BASE64_URL_SAFE } else { BASE64_CHARS };
	let bytes = input.as_ref();
	let len = bytes.len();
	let remainder = len % 3;
...
pub fn native_decode_base64(input: String) -> Vec<u8> {
	...
	if len == 0 { return vec![]; }
	let mut padding = 0;
	if len > 0 && bytes[len - 1] == b'=' { padding += 1; }
	if len > 1 && bytes[len - 2] == b'=' { padding += 1; }
	let olen = (len * 6) / 8 - padding;
	if olen == 0 { return vec![]; }
	...
	while i < len && bytes[i] != b'=' {
		...
		acc <<= (4 - cs) * 6;
		if cs >= 2 && pos < olen { out[pos] = (acc >> 16) as u8; pos += 1; }
		if cs >= 3 && pos < olen { out[pos] = (acc >> 8) as u8; pos += 1; }
		if cs >= 4 && pos < olen { out[pos] = acc as u8; pos += 1; }
		i += cs;
	}
	if pos != olen { return vec![]; }
	out
}
