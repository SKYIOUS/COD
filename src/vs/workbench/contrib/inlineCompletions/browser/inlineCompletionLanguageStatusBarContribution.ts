	...
	constructor(
		...
		this._register(autorunWithStore((reader, store) => {
			...
			const statusMap: Record<typeof status, { shortLabel: string; label: string; loading: boolean }> = {
				...
				inlineEdit: { shortLabel: '$(lightbulb-sparkle)', label: '$(lightbulb) ' + localize('inlineEditAvailable', "Inline edit available"), loading: false, },
				noSuggestion: { shortLabel: '$(circle-slash)', label: '$(lightbulb) ' + localize('noInlineSuggestionAvailable', "No inline suggestion available"), loading: false, },
			};
